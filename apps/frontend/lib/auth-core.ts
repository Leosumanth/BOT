...
export function verifyDashboardSessionToken(token: string): boolean {
  return verifySignedToken(token, DASHBOARD_SESSION_AUDIENCE, resolveDashboardSessionSecret());
}
